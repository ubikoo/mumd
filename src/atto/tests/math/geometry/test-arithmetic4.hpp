...
void test_arithmetic4 (void)
{
    ...
    { /* test abs */
        math::vec4<Type> a, abs_a;

        a = math::vec4<Type>(-3,-2,-1,0);
        abs_a = abs(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(abs_a(i), std::fabs(a(i))));
        }

        a = math::vec4<Type>(-2,-1,1,2);
        abs_a = abs(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(abs_a(i), std::fabs(a(i))));
        }
    ...
    { /* test sign */
        math::vec4<Type> a, sign_a;

        a = math::vec4<Type>(-2,-3,-4,-5);
        sign_a = sign(a);
        for (size_t i = 0; i < a.length(); i++) {
    ...
    { /* test floor */
        math::vec4<Type> a, floor_a;

        a = math::vec4<Type>(-2.1, -1.2, -0.8, -0.4);
        floor_a = floor(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(floor_a(i), std::floor(a(i))));
        }

        a = math::vec4<Type>(0.8, 0.4, 1.2, 2.1);
        floor_a = floor(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(floor_a(i), std::floor(a(i))));
        }
    ...
    { /* test round */
        math::vec4<Type> a, round_a;

        a = math::vec4<Type>(-2.3, -2.6, -1.3, -1.6);
        round_a = round(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(round_a(i), std::round(a(i))));
        }

        a = math::vec4<Type>(1.3, 1.6, 2.3, 2.6);
        round_a = round(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(round_a(i), std::round(a(i))));
        }
    ...
    { /* test clamp */
        ...
        math::vec4<Type> a, clamp_a;
        Type lo = -1.0;
        Type hi =  0.0;
        a = math::vec4<Type>(-1.5, -1.0, -0.5, 0.0);
        ...
        lo =  0.0;
        hi =  1.0;
        a = math::vec4<Type>(0.0, 0.5, 1.0, 1.5);
        clamp_a = clamp(a, lo, hi);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(math::isequal(clamp_a(i), clamp_fun(a(i), lo, hi)));
        }
    ...
    { /* test lerp */
        ...
        auto lerp_fun = [&] (
            const math::vec4<Type> &lo,
            const math::vec4<Type> &hi,
            const Type alpha) -> math::vec4<Type> {
            return (lo*(one - alpha) + hi*alpha);
        };

        math::vec4<Type> lo(-1.0);
        math::vec4<Type> hi( 0.0);
        Type alpha = 0.0;
        while (alpha < 1.0) {
            math::vec4<Type> a = lerp(lo,  hi, alpha);
            math::vec4<Type> b = lerp_fun(lo,  hi, alpha);
            math::vec4<Type> e = (a - b);
            for (size_t i = 0; i < a.length(); ++i) {
        ...
        lo = math::vec4<Type>(0.0);
        hi = math::vec4<Type>(1.0);
        alpha = 0.0;
        while (alpha < 1.0) {
            math::vec4<Type> a = lerp(lo,  hi, alpha);
            math::vec4<Type> b = lerp_fun(lo,  hi, alpha);
