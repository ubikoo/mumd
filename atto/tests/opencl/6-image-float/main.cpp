...
int main(int argc, char const *argv[])
{
    ...
    long stride = n_channels;

    std::vector<cl_float> src_data = cl::convert_image_to_float(image);
    std::vector<cl_float> dst_data(src_data);
    {
        uint8_t *px = image.bitmap();
        for (auto &it : dst_data) {
            *px++ = static_cast<uint8_t>(it * 255.0f);
        }
        std::fill(dst_data.begin(), dst_data.end(), cl_float());
    }
    ...
    buffers.emplace_back(cl::Memory::create_buffer(
        context,
        CL_MEM_WRITE_ONLY,
        dst_data.size() * sizeof(cl_float),
        (void *) NULL));

    buffers.emplace_back(cl::Memory::create_buffer(
        context,
        CL_MEM_READ_ONLY,
        src_data.size() * sizeof(cl_float),
        (void *) NULL));
        ...
        */
        {
            ...
            cl::Kernel::set_arg(kernel, 4, sizeof(cl_long), &stride);
